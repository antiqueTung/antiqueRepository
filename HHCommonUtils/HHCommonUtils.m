...
+(void)showError:(NSError *)error inErrorArrar:(NSArray<NSString *> *)errorArray {
    ...
    for(NSString *currentError in errorArray) {
        if([currentError isEqualToString:errorDescription]) {
...
+(NSString *)dealWithErrorInfo:(NSError *)error withErrorCode:(NSString *)errorCode andDealType:(NSString*)dealType{
    NSString *errorDescription = [error.userInfo objectForKey:@"NSLocalizedDescription"];
    if((![errorDescription isEqual:[NSNull null]])&&[errorDescription isEqualToString:errorCode]) {
        if([dealType isEqualToString:@"return"]) {
